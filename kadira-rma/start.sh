...
while [[ true ]]; do
  ...
  if [ -z ${START_TIME+x} ] || [ -z ${END_TIME+x} ]; then
      ...
      node server.js --mongo-url $MONGO_METRICS_URL --profile $PROFILE --provider $PROVIDER $ENV_FILE_NAME --type incremental lib.js mapreduce.js incremental-aggregation.js;
  else
      ...
      node server.js $MONGO_METRICS_URL profiles/$PROFILE.js providers/$PROVIDER.js $ENV_FILE_NAME lib.js mapreduce.js batch-aggregation.js;
